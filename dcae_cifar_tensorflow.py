...
W_e_conv5 = tf.Variable(xavier_init([4,4,384,latent_size]))
b_e_conv5 = tf.Variable(tf.zeros(shape=[latent_size]))

#######

W_d_dconv1 = tf.Variable(xavier_init([4,4,384,latent_size]))
...
def Encoder(x):
    ...
    h_e_re5 = tf.reshape(h_e_conv5, [-1, tf.shape(h_e_conv5)[3]])

    return h_e_re5

def Decoder(z):
    h_d_re = tf.reshape(z, [-1, 1, 1, tf.shape(z)[1]])
