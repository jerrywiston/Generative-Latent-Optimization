...
#Model Variable
W_latent = tf.Variable(xavier_init([samp_size, 500]))
b_latent = tf.Variable(tf.zeros(shape=[500]))

W_mu = tf.Variable(xavier_init([500, latent_size]))
b_mu = tf.Variable(tf.zeros(shape=[latent_size]))
W_sigma = tf.Variable(xavier_init([500, latent_size]))
b_sigma = tf.Variable(tf.zeros(shape=[latent_size]))

W_g1 = tf.Variable(xavier_init([latent_size,100]))
b_g1 = tf.Variable(tf.zeros(shape=[100]))
W_g2 = tf.Variable(xavier_init([100,128]))
...
def OneHotEncoder(Z_o):
    h = tf.nn.relu(tf.matmul(Z_o,W_latent) + b_latent, name='latent')
    z_mu = tf.matmul(h, W_mu) + b_mu
    z_logvar = tf.matmul(h, W_sigma) + b_sigma
